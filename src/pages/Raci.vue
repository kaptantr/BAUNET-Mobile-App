    ...
    <q-pull-to-refresh @refresh="refreshTasks">
      <div class="dashboard q-pa-sm bg-white">
        ...
        <q-list bordered padding class="rounded-borders">
          <q-item-label header>RACI ({{ count }}):</q-item-label>

          <q-item clickable v-ripple v-for="item in raciList" :key="item['X_SECURITY_HASH']" style="width: 95vw;display:block"  class="q-pa-none q-ma-none">
            <q-expansion-item :default-opened="raciList.length==1 ? true : false">
              <template v-slot:header>
                <q-item-section avatar top><q-avatar icon="person" text-color="blue-8" /></q-item-section>
                <q-item-section>
                  <q-item-label lines="1">{{ item['s_USER_FIRSTNAME'] + ' ' + item['s_USER_LASTNAME'] }}</q-item-label>
              ...
              <div class="q-pa-md">
                <table style="width:100%">
                  ...
                  <tr> <td><b>Benutzer Email:</b></td> <td>{{ item['s_USER_EMAIL'] }}</td> </tr>
                </table>
              </div>
            </q-expansion-item>
          </q-item>
...
export default defineComponent({
  ...
  methods: {
    ...
    requestData() {
      ...
      LocalStorage.remove('RaciList');
      Helpers.apiSubRequest('/WebServices/tasks/' + this.task_id + '/raci?limit=100&offset=00', 'Raci');
      let timerFunc = setInterval(() => {
        if(LocalStorage.has('RaciList')) {
          this.loadedPage = false;
          this.raciList = JSON.parse(LocalStorage.getItem('RaciList'));
  ...
  watch: {
    raciList() {
      this.count = this.raciList.length;
    }
  }
});
</script>
