    ...
    <q-pull-to-refresh @refresh="refreshTasks">
      <div class="dashboard q-pa-sm bg-white">
        ...
        <q-list bordered padding class="rounded-borders">
          <q-item-label header>TEAMS ({{ count }}):</q-item-label>

          <q-item clickable v-ripple v-for="item in teamsList" :key="item['X_SECURITY_HASH']" style="width: 95vw;display:block"  class="q-pa-none q-ma-none">
            <q-expansion-item :default-opened="teamsList.length==1 ? true : false">
              <template v-slot:header>
                <q-item-section avatar top><q-avatar icon="groups" text-color="blue-8" /></q-item-section>
                <q-item-section>
                  <q-item-label lines="1">{{ item['s_CLIENT_TEAM'] }}</q-item-label>
                </q-item-section>                
...
export default defineComponent({
  ...
  methods: {
    ...
    requestData() {
      ...
      LocalStorage.remove('TeamsList');
      Helpers.apiSubRequest('/WebServices/tasks/' + this.task_id + '/teams?limit=100&offset=00', 'Teams');
      let timerFunc = setInterval(() => {
        if(LocalStorage.has('TeamsList')) {
          this.loadedPage = false;
          this.teamsList = JSON.parse(LocalStorage.getItem('TeamsList'));
  ...
  watch: {
    teamsList() {
      this.count = this.teamsList.length;
    }
  }
});
</script>
